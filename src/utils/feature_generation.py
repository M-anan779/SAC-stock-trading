...
class TechnicalIndicators:
    ...
    def _add_directional_features(self):
        ...
        self.df["ADX"] = (adx["ADX_14"] / 100).clip(0, 1)
    ...
    def _add_trend_confirmation_features(self):
        ...
        self.df["time_since_reversal"] = (self._time_since_reversal_helper(series=smi, upper_thresh=40, lower_thresh=-40) / 390).clip(0, 1)
    ...
    # price z score + information content (used in Shannon Entropy)
    def _add_price_features(self):
        ...
        # standard normal CDF
        calculate_phi = lambda x: 0.5 * (1.0 + special.erf(x / np.sqrt(2.0)))
        phi = 2.0 * (1.0 - calculate_phi(abs(self.df["price_zscore"])))                                         # two-sided tail prob calculated using
        information = (-np.log(np.maximum(phi, 1e-8)) / 15).clip(-1, 1)                                                # information content, how "surprising" a price jump is
        self.df["entropy"] = information

    def _add_volatility_zscore(self):
        returns = self.df["close"].pct_change()
        vol = returns.rolling(30).std()
        rolling_mean = vol.rolling(30).mean()
        rolling_std = vol.rolling(30).std(ddof=0)
        self.df["volatility_zscore"] = ((vol - rolling_mean) / rolling_std.replace(0, 1e-8)).clip(-5, 5)

    # normalize z score columns through tanh
    def _normalize(self):
        cols = ["DMI_dev", "SMI_dev_delta"]
        ...
        self.df[cols] = np.tanh(self.df[cols]).clip(-1, 1)
